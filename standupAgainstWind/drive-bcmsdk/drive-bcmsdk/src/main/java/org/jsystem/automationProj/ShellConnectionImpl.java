...
public abstract class ShellConnectionImpl extends SystemObjectImpl implements
	...
	private void internalConnect() throws Exception {
		...
		report.report("Init cli, host: " + host + " port " + port);
		if (dummy) {
			return;
		}
		// Terminal t;
		...
		} else if (protocol.toLowerCase().equals(EnumConnectionType.SSH2.value())) {
			terminal = new SSH2(host, user, password, port);
			prompts.add(new Prompt("$", false, true));
			prompts.add(new Prompt("]$", false, true));
			Prompt[] promptsPrint = getAllPrompts();
			for (int i = 0; i < promptsPrint.length; i++) {
				System.out.println(promptsPrint[i].getPrompt());
			}
		} else if (protocol.toLowerCase().equals(EnumConnectionType.SSH2_RSA.value())) {
			terminal = new SSH2WithRSA(host, user, password, port, privateKey);
			prompts.add(new Prompt("$", false, true));
			prompts.add(new Prompt("]$", false, true));
			Prompt[] promptsPrint = getAllPrompts();
			for (int i = 0; i < promptsPrint.length; i++) {
